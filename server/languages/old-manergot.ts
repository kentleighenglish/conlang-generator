...
export class OldManergot implements LanguageClass {
  ...
  public translate(germanWord: string): Translation[] {
    ...
    if (Math.random() < chaos * 0.8) {
      c = c.replace(/[aeiouy]/g, m => this._randChoice(['ai', 'au', 'oi', 'ei', 'ou', 'ui', m], chaos * 0.7));
    }
    ...
    if (Math.random() < chaos * 0.5) {
      c = c.replace(/n[kg]/g, 'ŋ').replace(/n/g, this._randChoice(['n', 'm', 'ŋ', ''], chaos));
    }

    // Final consonant drop or epenthesis
    if (Math.random() < chaos * 0.7) {
      if (c.match(/[bcdfghjklmnpqrstvwxz]$/)) {
        c = c.slice(0, -1) + this._randChoice(['', 'a', 'e', 'i', 'o', 'h', 'n'], chaos);
      }
    }
    ...
    if (Math.random() < chaos * 0.3) {
      ...
      c = this._randChoice(prefixes, chaos) + c + this._randChoice(suffixes, chaos);
    }
